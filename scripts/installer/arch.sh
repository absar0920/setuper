...
if [ "$REACT_NATIVE" == "true" -o "$OPTIONS" == "all" ]; then
    ...
    sudo -u $MACHINE_USER yay -S --noconfirm android-studio
...
if [ "$POSTMAN" == "true" -o "$OPTIONS" == "all" -o "$MINE" == "true" -o "$RECOMMENDED" == "true" ]; then
    ...
    sudo -u $MACHINE_USER yay -S --noconfirm postman-bin
fi

if [ "$MINE" == "true" -o "$WHATSAPP" == "true" -o "$RECOMMENDED" == "true" -o "$OPTIONS" == "all" ]; then
    echo "INSTALLING WHATSAPP..."
    sudo -u $MACHINE_USER yay -S --noconfirm whatsie whatsdesk-bin
fi
...
if [ "$MINE" == "true" ]; then
    ...
    sudo -u $MACHINE_USER yay -S --noconfirm thorium-browser-bin
    sudo -u $MACHINE_USER yay -S --noconfirm opera
